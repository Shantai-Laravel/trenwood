        ...
        <div class="list-content part full-part">
            ...
            <div class="form-group">
                <label>
                    ...
                    ({{ $property->translationByLanguage($lang->id)->first()->unit }})
            ...
            @elseif ($property->type == 'text')
                <div class="row">
                    @foreach ($langs as $key => $oneLang)
                        <div class="col-md-6">
                            [{{ $oneLang->lang }}]<input type="text" name="propText[{{ $property->id }}][{{ $oneLang->id }}]" value="{{ getPropertiesData(Request::segment(3), $property->id, $oneLang->id) ?? $property->translation->first()->value }}" class="form-control">
                        </div>
                    @endforeach
                </div>
