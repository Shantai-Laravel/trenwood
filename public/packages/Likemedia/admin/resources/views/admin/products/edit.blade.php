        ...
        <div class="row">
           ...
           <div class="col-md-3">
                <ul>
                    <li>
                        <label>Seturi</label>
                        {{-- <select multiple name="set_id[]" class="form-control">
                        <option value="0">---</option>
                        @foreach($sets as $set)
                            <option value="{{ $set->id }}" {{ !is_null($set->setProduct($product->id)->first()) ? 'selected' : '' }}>{{ $set->translationByLanguage()->first()->name }}</option>
                        @endforeach
                        </select> --}}
        ...
        <div class="tab-content {{ $loop->first ? ' active-content' : '' }}" id={{ $lang->
            ...
            <div class="part left-part">
                <ul style="padding: 25px 0;">
                    ...
                    <li>
                      <label for="">Videos [{{$lang->lang}}]</label>
                      @if ($product->videos)
                          @foreach ($product->videos as $key => $video)
                              @if ($video->translationByLanguage($lang->id)->first()->src !== null)
                                <div class="video-wrapp-full wrapp-full">
                                    <section>
                                        <a href="{{ url('back/products/video/delete/'. $video->id) }}" class="close"><i class="fa fa-times-circle"></i> </a>
                                    </section>
                                    <iframe width="100%" height="100%" src="https://www.youtube.com/embed/{{getYouTubeId($video->translationByLanguage($lang->id)->first()->src)}}" frameborder="0" allow="accelerometer; autoplay="1"; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                                </div>
                              @endif
                          @endforeach
                      @endif
                      <input type="text" name="video_{{$lang->lang}}" value="">
                    </li>
                </ul>
                            ...
                            @if (!empty($lifestyleImages))
                                @foreach ($lifestyleImages as $key => $lifestyleImage)
                                    <div class="row image-list">
                                        ...
                                        <div class="col-md-1">
                                            <a href="#" class="main-btn" data-id="{{ $lifestyleImage->id }}"><i class="fa fa-check"></i></a>
