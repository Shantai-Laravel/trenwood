...
<div class="title-block">
    <h3 class="title"> {{trans('front.admin.updateReturn')}} </h3>
...
<div class="list-content">
    ...
    <div class="col-md-12">
      ...
      <div class="col-md-4">
        <label>{{trans('front.admin.chooseStatus')}}</label>
        <select class="form-control" name="status_return">
            <option {{!empty($return) &&$return->status == 'new' ? 'selected' : ''}} value="new">{{trans('front.cabinet.status.new')}}</option>
            <option {{!empty($return) &&$return->status == 'processing' ? 'selected' : ''}} value="processing">{{trans('front.cabinet.status.processing')}}</option>
            <option {{!empty($return) &&$return->status == 'cancelled' ? 'selected' : ''}} value="cancelled">{{trans('front.cabinet.status.cancelled')}}</option>
            <option {{!empty($return) &&$return->status == 'completed' ? 'selected' : ''}} value="completed">{{trans('front.cabinet.status.completed')}}</option>
        </select>
      </div>
    </div>
