    ...
    <div class="card-block row">
            ...
            <table class="table table-hover table-striped" id="tablelistsorter">
                ...
                <tbody>
                    ...
                    <tr id="{{ $collection->id }}">
                        ...
                        <td>
                            {{ $collection->translationByLanguage()->first()->name ?? trans('variables.another_name') }}
                        </td>
        ...
        <div class="col-md-3">
            ...
            @foreach($collections as $key => $collection)
                <ul class="list-tree">
                    <li>
                        <a href="{{ route('collections.edit', $collection->id) }}">- {{ $collection->translationByLanguage()->first()->name }}</a>
                        @if (count($collection->sets()->get()) > 0)
                            <ol>
                                @foreach ($collection->sets()->get() as $key => $set)
                                    <li><a href="{{ route('sets.edit', $set->id) }}">--  {{ $set->translationByLanguage()->first()->name }}</a></li>
                                @endforeach
                            </ol>
                        @endif
                    </li>
