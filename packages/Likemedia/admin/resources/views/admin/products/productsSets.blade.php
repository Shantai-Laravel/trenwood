...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ url('/back/sets/collection/'.$set->collection_id) }}">{{ $set->translationByLanguage($lang->id)->first()->name }}</a></li>
...
<div class="title-block">
    <h3 class="title"> Produse
        @if (!is_null($set))
            @if (!is_null($set->translationByLanguage($lang->id)->first()))
                 {{ $set->translationByLanguage($lang->id)->first()->name }}
            @endif
        @endif
    </h3>
        ...
        <table class="table table-hover table-striped" id="tablelistsorter">
            ...
            <tbody>
                ...
                <tr id="{{ $product->id }}">
                    <td>{{ $key + 1 }}</td>
                    <td>
                        @if (!is_null($product->translationByLanguage($lang->id)->first()))
